...
export default function DashboardEmptyState({
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-rose-100 bg-rose-50/40 px-4 py-8 text-center">
        ...
        <p className="mt-1 max-w-xs text-[11px] text-rose-600 leading-relaxed">
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/50 px-4 py-8 text-center">
      ...
      <p className="mt-1 max-w-xs text-[11px] text-slate-500 leading-relaxed">{description}</p>
      {actionLabel && onAction && (
        <button
          type="button"
          onClick={onAction}
          className="mt-3 rounded-lg border border-slate-200 bg-white px-3.5 py-1.5 text-xs font-semibold text-slate-700 shadow-xs transition-colors hover:bg-slate-50 hover:text-emerald-700"
        >
          {actionLabel}
        </button>
      )}
