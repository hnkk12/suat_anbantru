...
function PermissionCheck({ action, checked, onChange }) {
  ...
  return <label className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50">
    ...
    <span className={`flex h-4 w-4 items-center justify-center rounded border ${checked ? 'border-emerald-600 bg-emerald-600 text-white' : 'border-slate-300'}`}>{checked && <Check size={11} strokeWidth={3} />}</span>
    <Icon size={13} className={checked ? 'text-emerald-600' : 'text-slate-400'} />{action.label}
  </label>
}
...
export default function MenuPermissions() {
  ...
  return <div className="space-y-5 pb-24">
    {toast && <div className={`fixed bottom-6 right-6 z-50 rounded-xl px-4 py-3 text-xs font-semibold text-white shadow-xl ${toast.ok ? 'bg-emerald-700' : 'bg-rose-700'}`}>{toast.text}</div>}
    <header><div className="flex items-center gap-2"><h1 className="text-2xl font-bold text-slate-900">Phân quyền menu</h1><span className="rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-xs font-semibold text-emerald-700">Quản trị bảo mật</span></div><p className="mt-1 text-sm text-slate-500">Thiết lập quyền thao tác trên từng module cho tài khoản trong hệ thống bán trú.</p></header>
    <div className="grid items-start gap-5 lg:grid-cols-[300px_minmax(0,1fr)]">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 p-4"><div><h2 className="text-sm font-bold">Tài khoản</h2><p className="text-[11px] text-slate-500">Chọn tài khoản để cấu hình quyền</p></div><button onClick={() => setAddOpen(true)} className="flex items-center gap-1 rounded-lg bg-emerald-600 px-2.5 py-1.5 text-xs font-semibold text-white"><Plus size={13} /> Thêm mới</button></div>
        <div className="p-3"><div className="relative"><Search className="absolute left-3 top-2.5 text-slate-400" size={14} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Tìm tên, username, vai trò..." className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-8 pr-3 text-xs outline-none focus:border-emerald-500" /></div>
          <div className="mt-3 max-h-[590px] space-y-1 overflow-y-auto">{accounts.length === 0 && <p className="py-10 text-center text-xs text-slate-400">Chưa có tài khoản</p>}{accounts.length > 0 && filtered.length === 0 && <p className="py-10 text-center text-xs text-slate-400">Không tìm thấy tài khoản phù hợp</p>}
            {filtered.map((a) => <button key={a.id} onClick={() => selectAccount(a.id)} className={`flex w-full items-center gap-3 rounded-xl border p-3 text-left ${selectedId === a.id ? 'border-emerald-400 bg-emerald-50' : 'border-transparent hover:border-slate-200 hover:bg-slate-50'}`}><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-xs font-bold ${selectedId === a.id ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600'}`}>{a.initials}</span><span className="min-w-0 flex-1"><b className="block truncate text-xs">{a.name}</b><span className="block truncate text-[11px] text-slate-500">{a.role}</span></span><ChevronRight size={14} className="text-slate-300" /></button>)}
          </div></div>
      </section>
      <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-100 p-5 xl:flex-row xl:items-center xl:justify-between"><div><h2 className="text-lg font-bold">{selected?.name || 'Chưa chọn tài khoản'}</h2><p className="text-xs text-slate-500">{selected ? `${enabledMenus} menu đang được thiết lập quyền` : 'Chọn một tài khoản ở danh sách bên trái'}</p></div>{selected && <div className="flex flex-wrap gap-1.5">{ACTIONS.map((a) => <span key={a.key} className="rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px]">{a.label}: <b className="text-emerald-700">{summary[a.key]}</b></span>)}</div>}</div>
