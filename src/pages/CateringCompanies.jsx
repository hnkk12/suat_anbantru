      ...
      {detailedLegal ? (
        <div className="space-y-4 rounded-xl border border-slate-200 bg-slate-50/60 p-4 sm:col-span-2">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <label className="flex flex-col gap-1 text-sm"><span className="text-xs font-medium text-gray-500">File quyết định</span><input type="file" onChange={(e) => selectFile('quyetDinhFile', e.target.files?.[0])} className="block w-full text-xs text-slate-500 file:mr-3 file:rounded-lg file:border-0 file:bg-emerald-50 file:px-3 file:py-2 file:text-xs file:font-medium file:text-emerald-700" />{form.quyetDinhFile && <span className="flex items-center gap-1 text-xs text-emerald-700"><Paperclip size={12} />{form.quyetDinhFile.name}</span>}</label>
            <div>
            ...
            <label className="flex flex-col gap-1 text-sm"><span className="text-xs font-medium text-gray-500">File giấy chứng nhận ATTP</span><input type="file" onChange={(e) => selectFile('attpFile', e.target.files?.[0])} className="block w-full text-xs text-slate-500 file:mr-3 file:rounded-lg file:border-0 file:bg-emerald-50 file:px-3 file:py-2 file:text-xs file:font-medium file:text-emerald-700" />{form.attpFile && <span className="flex items-center gap-1 text-xs text-emerald-700"><Paperclip size={12} />{form.attpFile.name}</span>}</label>
          </div>
        </div>
      ) : (
        <div className="sm:col-span-2">
...
function PartnerList({ title, partners, emptyLabel }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50/40 p-4">
      ...
      {partners.length === 0 ? <p className="py-7 text-center text-xs text-slate-400">{emptyLabel}</p> : <div className="mt-3 space-y-2">{partners.map((partner, index) => <div key={partner.id || index} className="rounded-lg border border-slate-200 bg-white p-3"><p className="text-xs font-semibold text-slate-800">{partner.ten || partner.name || 'Chưa cập nhật tên'}</p><p className="mt-1 text-[11px] text-slate-500">{partner.mst ? `MST: ${partner.mst} · ` : ''}{partner.sdt || partner.phone || 'Chưa cập nhật số điện thoại'}</p><p className="mt-1 text-[11px] text-slate-400">{partner.diaChi || partner.address || 'Chưa cập nhật địa chỉ'}</p></div>)}</div>}
    </div>
  )
}
...
function AttachmentInfo({ label, file }) {
  ...
  return <p className="mt-2 flex items-center gap-1.5 text-xs font-medium text-emerald-700"><Paperclip size={13} /><span className="truncate">{label}: {file.name}{size}</span></p>
}
...
function MealProviderSection({ company, onEdit, onRemove }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="flex flex-col gap-4 border-b border-slate-100 p-5 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700"><Building2 size={20} /></span><div><h2 className="text-base font-bold text-slate-900">{company.ten}</h2><Badge tone={isActive ? 'green' : 'red'} className="mt-1">{modelName}</Badge></div></div><div className="mt-4 grid gap-1 text-xs text-slate-500 sm:grid-cols-2"><p>MST: {company.mst || 'Chưa cập nhật'}</p><p>Điện thoại: {company.sdt || 'Chưa cập nhật'}</p><p className="sm:col-span-2">Địa chỉ: {company.diaChi || 'Chưa cập nhật'}</p>{company.moTa && <p className="sm:col-span-2">Mô tả: {company.moTa}</p>}</div></div>
      <div className="flex gap-1"><button onClick={() => onEdit(company)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-emerald-700"><Pencil size={16} /></button><button onClick={() => onRemove(company)} className="rounded-lg p-2 text-slate-400 hover:bg-rose-50 hover:text-rose-600"><Trash2 size={16} /></button></div>
    </div>
...
export default function CateringCompanies() {
  ...
  return (
    <div className="space-y-6">
      {toast && <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-medium text-white shadow-xl ${toast.ok ? 'bg-emerald-700' : 'bg-rose-700'}`}>{toast.ok && <CheckCircle2 size={16} />}{toast.text}</div>}
      <PageHeader eyebrow="Quản trị" eyebrowIcon={Building2} title="Công ty cung cấp suất ăn" description="Quản lý đơn vị cung cấp suất ăn bán trú và thông tin đối tác hợp tác cùng nhà trường." />
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row"><div className="relative flex-1"><Search size={15} className="absolute left-3 top-2.5 text-slate-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Tìm theo tên công ty, MST hoặc số điện thoại..." className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-xs outline-none focus:border-emerald-500" /></div><select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600 outline-none"><option>Tất cả</option><option>Đang hợp tác</option><option>Tạm ngưng</option></select></div>
      </section>
      ...
      <Modal open={selectOpen} onClose={() => setSelectOpen(false)} title="Chọn công ty" width="max-w-3xl" footer={<><Button variant="secondary" onClick={() => setSelectOpen(false)} disabled={saving}>Hủy</Button><Button onClick={saveSelection} disabled={JSON.stringify([...draftProviderIds].sort()) === JSON.stringify([...assignedProviderIds].sort()) || saving}>{saving ? 'Đang lưu...' : 'Lưu lựa chọn'}</Button></>}>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">{companies.map((company) => { const checked = draftProviderIds.includes(company.id); return <button key={company.id} onClick={() => setDraftProviderIds((ids) => checked ? ids.filter((id) => id !== company.id) : [...ids, company.id])} className={`rounded-xl border p-4 text-left transition-colors ${checked ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 hover:border-emerald-200'}`}><div className="flex justify-between gap-2"><b className="text-sm text-slate-900">{company.ten}</b><Badge tone={checked ? 'green' : 'gray'}>{checked ? 'Đã chọn' : 'Chọn đơn vị'}</Badge></div><p className="mt-2 text-xs text-slate-500">{company.sdt || 'Chưa có số điện thoại'}</p><p className="mt-1 text-xs text-slate-400">{company.diaChi || 'Chưa có địa chỉ'}</p></button> })}</div>
      </Modal>
