...
export default function Menu() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Thực đơn" description="Theo dõi và cập nhật thực đơn bán trú. Chọn một bữa ăn để chỉnh sửa món ăn." />
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="inline-flex w-fit rounded-xl bg-slate-100 p-1">
            <button onClick={() => setView('day')} className={`rounded-lg px-3 py-2 text-xs font-semibold transition-colors ${view === 'day' ? 'bg-white text-teal-700 shadow-sm' : 'text-slate-500'}`}>Theo ngày</button>
            <button onClick={() => setView('week')} className={`rounded-lg px-3 py-2 text-xs font-semibold transition-colors ${view === 'week' ? 'bg-white text-teal-700 shadow-sm' : 'text-slate-500'}`}>Theo tuần</button>
          </div>
          <span className="flex items-center gap-1.5 text-xs text-slate-500"><UtensilsCrossed size={15} className="text-teal-600" />{MEALS.length} buổi ăn · {DAYS_OF_WEEK.length} ngày trong tuần</span>
        </div>
      </section>
